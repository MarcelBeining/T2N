...
[tree,params,neuron,thesetrees,usestreesof] = t2n_checkinput(tree,params,neuron);
...
for n = 1:numel(neuron)
    ...
    for tt = 1:numel(tree(thesetrees{n}))
        if ~isfield(tree{thesetrees{n}(tt)},'artificial')
            x = t2n_getref(n,neuron,'mech');
            minterf{thesetrees{n}(tt)} = t2n_make_nseg(tree{thesetrees{n}(tt)},origminterf{thesetrees{n}(tt)},params,neuron{x}.mech{thesetrees{n}(tt)});
        end
    end
    ...
    fprintf(nfile,'\n\n');
    if params.parallel
        [GIDs,neuron,mindelay] = t2n_getGIDs(neuron{n},trees,thesetrees{n});
        ...
        if params.parallel
            for g = 1:numel(GIDs)
                ...
                if isfield(tree{thiscell},'artificial')
                    ...
                    fprintf(ofile,'cell.allregobj.o(%d).sec {con = new NetCon(&%s(0.5),nil)}\n',minterf{thesetrees{n}(tt)}(inode,2),GIDs(g).watch);  % make temporary netcon for registering the cell
                end
                    ...
                    if params.cvode
                        if params.use_local_dt  % if yes, dt was different for each cell, so there is more than one time vector
                            if numel(out{readfiles{f}{2}}.t) < readfiles{f}{3} || isempty(out{readfiles{f}{2}}.t{readfiles{f}{3}})
                                out{readfiles{f}{2}}.t{readfiles{f}{3}} = load(fullfile(exchfolder,sprintf('sim%d',readfiles{f}{2}),sprintf('cell%d_tvec.dat', find(readfiles{f}{3} == thesetrees{n})-1)),'-ascii');    %loading of one time vector file per cell (sufficient)
                            end
